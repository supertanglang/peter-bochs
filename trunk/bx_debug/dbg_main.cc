...
static struct {
	...
	unsigned next_super_breakpoint_id;
}bx_debugger;
...
void print8Bytes(unsigned char c[8]) {
	dbg_printf(", byte=");
	for (int x=0;x<8;x++) {
		dbg_printf("%x ",c[x]);
	}
	...
	// print GDT detail
	// end print GDT detail
}

unsigned int convert2(unsigned char c[2]) {
	return (c[1]<<8)|(c[0]);
}

unsigned int convert4(unsigned char c[4]) {
...
void bx_dbg_print_descriptor_extra(unsigned char desc[8])
{
	Bit32u lo = (desc[3] << 24) | (desc[2] << 16) | (desc[1] << 8) | (desc[0]);
	Bit32u hi = (desc[7] << 24) | (desc[6] << 16) | (desc[5] << 8) | (desc[4]);
	Bit32u base = ((lo >> 16) & 0xffff)
	...
	Bit32u offset = (lo & 0xffff) | (hi & 0xffff0000);
	unsigned type = (hi >> 8) & 0xf;
	unsigned avl=(hi>>20) & 0x1;
	unsigned p = (hi >> 15) & 0x1;
	unsigned dpl = (hi >> 13) & 0x3;
	unsigned s = (hi >> 12) & 0x1;
	unsigned d_b = (hi >> 22) & 0x1;
	unsigned g = (hi >> 23) & 0x1;

	unsigned bit40=(hi >> 8) & 0x1;
	unsigned bit41=(hi >> 9) & 0x1;
	unsigned bit42=(hi >> 10) & 0x1;
	unsigned bit43=(hi >> 11) & 0x1;
	...
	if (s) {
		...
		if (type&8) {
			...
			print8Bytes(desc);
			dbg_printf("\tbase=%x, limit=%x%s, g=%x,  d=%x,, avl=%x, p=%x, dpl=%x, s=%x, type=%x\n\n", base, limit, g ? "* 4Kb" : "", g, d_b, avl, p, dpl, s, type);
			...
			print8Bytes(desc);
			dbg_printf("\tbase=%x, limit=%x%s, g=%x,  b=%x, avl=%x, p=%x, dpl=%x, s=%x, type=%x\n\n", base, limit, g ? "* 4Kb" : "", g, d_b, avl, p, dpl, s, type);
		}
	} else {
		if (type==2) {
			// LDT
			dbg_printf("LDT");
			print8Bytes(desc);
			dbg_printf("\tbase=%x, limit=%x%s, g=%x, avl=%x, p=%x, dpl=%x, s=%x, type=%x\n\n", base, limit, g ? "* 4Kb" : "", g, avl, p, dpl, s, type);
		} else if (bit40==1 && bit42==0 && s==0) {
			if (bit43==1) {
				dbg_printf("32 bit TSS");
				Bit8u tempBuffer[100];
				bx_dbg_read_linear(dbg_cpu, base, 100, tempBuffer);
				print8Bytes(desc);
				dbg_printf("\tbase=%x, limit=%x%s, g=%x, avl=%x, p=%x, dpl=%x, s=%x, d=%x, b=%x, type=%x\n\n", base, limit, g ? "* 4Kb" : "", g, avl, p, dpl, s, bit43, bit41, type);

				for (int y=0;y<4;y++) {
					dbg_printf("\t%0.2d : ",y*16);
					for (int x=0;x<16;x++) {
						dbg_printf("%0.2x ", tempBuffer[y*16+x]);
					}
					dbg_printf("\n");
				}

				// print TSS in human reading
				dbg_printf("\n");
				dbg_printf("\tLink=%0.8x ", convert2(&tempBuffer[0]));
				dbg_printf("ESP0=%0.8x ", convert4(&tempBuffer[4]));
				dbg_printf("SS0=%0.8x ", convert2(&tempBuffer[8]));
				dbg_printf("ESP1=%0.8x ", convert4(&tempBuffer[12]));
				dbg_printf("SS1=%0.8x ", convert2(&tempBuffer[16]));
				dbg_printf("ESP2=%0.8x ", convert4(&tempBuffer[20]));
				dbg_printf("SS2=%0.8x ", convert2(&tempBuffer[24]));
				dbg_printf("\n");
				dbg_printf("\tCR3=%0.8x ", convert4(&tempBuffer[28]));
				dbg_printf("EIP=%0.8x ", convert4(&tempBuffer[32]));
				dbg_printf("EFLAGS=%0.8x ",convert4(&tempBuffer[36]));
				dbg_printf("\n");
				dbg_printf("\tEAX=%0.8x ",convert4(&tempBuffer[40]));
				dbg_printf("ECX=%0.8x ",convert4(&tempBuffer[44]));
				dbg_printf("EDX=%0.8x ",convert4(&tempBuffer[48]));
				dbg_printf("EBX=%0.8x ",convert4(&tempBuffer[52]));
				dbg_printf("ESP=%0.8x ",convert4(&tempBuffer[56]));
				dbg_printf("EBP=%0.8x ",convert4(&tempBuffer[60]));
				dbg_printf("ESI=%0.8x ",convert4(&tempBuffer[64]));
				dbg_printf("EDI=%0.8x ",convert4(&tempBuffer[68]));
				dbg_printf("\n");
				dbg_printf("\tES=%0.8x ",convert2(&tempBuffer[72]));
				dbg_printf("CS=%0.8x ",convert2(&tempBuffer[76]));
				dbg_printf("SS=%0.8x ",convert2(&tempBuffer[80]));
				dbg_printf("DS=%0.8x ",convert2(&tempBuffer[84]));
				dbg_printf("FS=%0.8x ",convert2(&tempBuffer[88]));
				dbg_printf("GS=%0.8x ",convert2(&tempBuffer[92]));
				dbg_printf("Task's LDT selector=%0.8x ",convert2(&tempBuffer[96]));
				dbg_printf("\n");
				// end print TSS in human reading
			} else {
				dbg_printf("16 bit TSS");
			}
...
void bx_dbg_gdt_command(unsigned from, unsigned to)
{
	...
	for (unsigned n = from; n<=to; n++) {
		...
		if (bx_dbg_read_linear(dbg_cpu, gdtr.base + 8*n, 8, entry)) {
			dbg_printf("GDT[0x%02x]=", n);
			bx_dbg_print_descriptor_extra(entry);
		}
		else {
			dbg_printf("error: GDTR+8*%d points to invalid linear address 0x" FMT_ADDRX "\n",
					n, gdtr.base);
...
void bx_dbg_search_memory_command(bx_address from, bx_address length, bx_address value) {
	Bit8u tempBuffer[length];
	bx_dbg_read_linear(dbg_cpu, from, length, tempBuffer);
	for (int x=0;x<length;x++) {
		if (value<=0xff) {
			if (value==tempBuffer[x]) {
				dbg_printf("%0.8x ",from+x);
			}
		} else if (value<=0xffff) {
			if (value==convert2(&tempBuffer[x])) {
				dbg_printf("%0.8x ",from+x);
			}
		} else if (value<=0xffffffff) {
			if (value==convert4(&tempBuffer[x])) {
				dbg_printf("%0.8x ",from+x);
			}
		}
	}
