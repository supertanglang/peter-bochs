...
public class GDTLDTPanel extends JPanel {
	...
	public GDTLDTPanel(Application application, int type, int gdtNo) {
		...
		try {
			{
				this.setLayout(null);
				{
					...
					jScrollPane1.setBounds(12, 38, 576, 60);
			...
			}
			{
				...
				jScrollPane3.setBounds(305, 110, 283, 471);
			...
			Application.commandReceiver.setCommandResult("");
			...
			String result = Application.commandReceiver.getCommandResult();
			String lines[] = result.split(System.getProperty("line.separator"));
			long address = Long.parseLong(lines[0].replaceFirst("^.*base=0x", "").replaceFirst(",.*$", ""), 16);

			// get bytes
			Application.commandReceiver.setCommandResult("");
			Application.commandReceiver.setCommandNoOfLine(2);
			Application.sendCommand("xp /8x " + (address + (gdtNo * 8)));
			result = Application.commandReceiver.getCommandResult();
			lines = result.split(System.getProperty("line.separator"));

			String byteStr[] = lines[1].replaceFirst("^.*>:\t", "").split("\t");
			for (int x = 0; x < 8; x++) {
	...
	private void parseLDT() {
		try {
			...
			String result = application.commandReceiver.getCommandResult(500);
			String lines[] = result.split(System.getProperty("line.separator"));
	...
	private void parseTSSDescriptor() {
		try {
			...
			Application.commandReceiver.setCommandResult("");
			Application.commandReceiver.setCommandNoOfLine(2);
			Application.sendCommand("x /" + limit + "x " + base);
			String result2 = Application.commandReceiver.getCommandResult();
			String[] lines2 = result2.split(System.getProperty("line.separator"));
