...
RETVAL StartRemoteThread(HANDLE hRemoteProc, DWORD dwEntryPoint){
    ...
    // inject the thread
    hRemoteThread=CreateRemoteThread(hRemoteProc, NULL, 0, (LPTHREAD_START_ROUTINE)dwEntryPoint, (void *)CTRL_BREAK_EVENT, CREATE_SUSPENDED, NULL);
...
RETVAL AdvancedOpenProcess(DWORD dwPid, HANDLE * phRemoteProc) {
    ...
    *phRemoteProc=OpenProcess(NEEDEDACCESS, false, dwPid);
    if (NULL==*phRemoteProc) {
        ...
        rv=SetPrivilege(hThisProcToken, (char *)SE_DEBUG_NAME, true);
        if (EXIT_OK==rv) {
            bDebugPriv=true;
        }
...
error:
    ...
    if (true==bDebugPriv) {
        rv2=SetPrivilege(hThisProcToken, (char *)SE_DEBUG_NAME, false);
...
int GetProcId(char* ProcName)
{
    ...
    if( Process32First( hSnapshot, &pe32 ) )
    {
        do{
			//printf("%S\n",pe32.szExeFile);
			char str[51];
			wcstombs(str, pe32.szExeFile, 50);
...
int main(unsigned int nArgs, char ** rgszArgs) {
    ...
    /*if (1!=nArgs || (('/'==rgszArgs[1][0] || '-'==rgszArgs[1][0])
	...
	int pid=GetProcId("bochsdbg.exe");
	printf("%d\n",pid);
	printf("%s\n",rgszArgs[1]);
